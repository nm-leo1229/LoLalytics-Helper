...
### 조합 점수 총점 표기 구현 (총 조합 점수)
- 각 챔피언의 score 점수를 모두 더한 값을 표기
- 우리팀 슬롯 상대팀 슬롯 각각 아래에 표기
- 전체적인 밴픽을 잘했다 못했다의 판단 기준

### 챔피언 이름 오른쪽에 (score) 를 표기하도록 구현
- 챔피언이 바뀔 때마다 모두 재갱신
- 상대 핵심 픽 파악 및 우리팀 핵심 픽 파악 목적

### 가중치를 유저가 수정할 수 있도록 구현
- 나의 라인을 선택하면 처음엔 디폴트 값을 불러오고 값을 변경할 때마다 저장해두고 다음에도 동일한 값을 불러올 수 있도록 구현
- 챔피언 이름 오른쪽에 배치. 가중치 [  ]
- 시너지와 카운터의 가충지 디펜던시 제거. 따로 관리하도록 변경

### OP 듀오 개선
- 나의 라인과 함께할 라인을 선택할 수 있게 구현해서 바텀 뿐만 아니라 다른 라인 입장에서도 최고의 듀오를 찾을 수 있게 개선
